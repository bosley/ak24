...
#define CELLS_X                                                                \
  (CELL_WRAP ? (SCREEN_WIDTH / CELL_SIZE)                                      \
             : ((SCREEN_WIDTH + CELL_SIZE - 1) / CELL_SIZE))

/** @brief Number of cells that fit vertically */
#define CELLS_Y                                                                \
  (CELL_WRAP ? (SCREEN_HEIGHT / CELL_SIZE)                                     \
             : ((SCREEN_HEIGHT + CELL_SIZE - 1) / CELL_SIZE))
...
static void cell_click_handler(void *captured_ctx, void *invoke_args) {
  ...
  if (cell->clicked) {
    ...
    cell->color = (SDL_Color){(Uint8)(100 + (region * 37) % 155),
                              (Uint8)(100 + (region * 73) % 155),
                              (Uint8)(100 + (region * 111) % 155), 255};
    cell->clicked = 1;
  }
...
/**
 * @brief Free click event context (AK24_FREE wrapper for lambda ctx_free)
 */
static void free_click_event(void *ptr) { AK24_FREE(ptr); }

/**
 * @brief Initialize cell grid
 */
static int init_cells(void) {
  cells = AK24_ALLOC(TOTAL_CELLS * sizeof(cell_t));
  if (!cells) {
    fprintf(stderr, "Failed to allocate cell grid\n");
    return -1;
  }
  memset(cells, 0, TOTAL_CELLS * sizeof(cell_t));
  ...
  for (int y = 0; y < CELLS_Y; y++) {
    for (int x = 0; x < CELLS_X; x++) {
      ...
      cells[idx].rect = (SDL_Rect){.x = x * CELL_SIZE,
                                   .y = y * CELL_SIZE,
  ...
  AK24_LOG_INFO("Initialized %d cells (%dx%d) in %d regions (%dx%d)",
                TOTAL_CELLS, CELLS_X, CELLS_Y, TOTAL_REGIONS, REGIONS_X,
                REGIONS_Y);
  return 0;
}
...
static int init_region_pools(void) {
  ...
  AK24_LOG_INFO("Created %d region thread pools (1 worker each)",
                TOTAL_REGIONS);
  return 0;
}
...
static void handle_click(int mouse_x, int mouse_y) {
  ...
  click_event_t *event = AK24_ALLOC(sizeof(click_event_t));
  if (!event)
    return;
  ...
  ak_lambda_t *task =
      ak_lambda_new(cell_click_handler, event, free_click_event);
  if (!task) {
    AK24_FREE(event);
    return;
  }
...
APP_MAIN(app_main) {
  ...
  printf("  Regions: %dx%d (%d total, %dx%d cells each)\n", REGIONS_X,
         REGIONS_Y, TOTAL_REGIONS, REGION_SIZE, REGION_SIZE);
  ...
  window = SDL_CreateWindow("AK24 Cell Grid - Click cells!",
