...
Extract 1D sequences from N-D structures by fixing dimensions:
```c
ak_atom_cluster_t *slice = ak_atom_slice(cube, constraints);
```

### Path Operations

Traverse between two points in N-D space:
```c
ak_atom_cluster_t *path = ak_atom_path(atom_a, atom_b);
```

### Parallel Evaluation

Evaluate across multiple dimensions simultaneously for tensor operations and parallel computation.
