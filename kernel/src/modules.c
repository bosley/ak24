...
static AK_MUTEX g_manager_init_mutex = AK_MUTEX_INITIALIZER;
...
__attribute__((unused)) static void destroy_manager(module_manager_t *mgr) {
  ...
  AK_MUTEX_LOCK(&mgr->registry_mutex);
  ...
  AK_MUTEX_UNLOCK(&mgr->registry_mutex);
  AK_MUTEX_DESTROY(&mgr->registry_mutex);
...
#pragma GCC diagnostic pop
  ...
  */
  if (!verify_function_in_module(dl_handle, (void *)version_fn,
    ...
    if (instance->access_mutex) {
      AK_MUTEX_DESTROY(instance->access_mutex);
  ...
  */
  if (options->thread_safe) {
    instance->access_mutex = (AK_MUTEX *)AK24_ALLOC(sizeof(AK_MUTEX));
    if (!instance->access_mutex) {
    ...
    AK_MUTEX_INIT(instance->access_mutex);
  } else {
