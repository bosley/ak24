...
static void simple_task(void *ctx, void *args) {
  ...
  if (context) {
    AK_MUTEX_LOCK(&context->mutex);
    context->value++;
    context->called = 1;
    AK_MUTEX_UNLOCK(&context->mutex);
  }
}
...
static int test_enqueue_during_shutdown(void) {
  ...
  AK_THREAD shutdown_thread;
  AK24_THREAD_CREATE(&shutdown_thread, NULL, shutdown_helper, pool);
  ...
  AK24_THREAD_JOIN(shutdown_thread, NULL);
